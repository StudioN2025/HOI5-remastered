...
// Функция для преобразования мировых координат в индекс на канвасе
function worldToCanvasX(x) {
    return (x - worldBounds.minX) * CELL_SIZE;
}

function worldToCanvasY(y) {
    return (y - worldBounds.minY) * CELL_SIZE;
}
...
function updateWorldBounds() {
    ...
    if (!gridData || Object.keys(gridData).length === 0) {
        ...
        worldBounds = { minX: -50, maxX: 120, minY: -90, maxY: 60 };
        return;
    }
...
export function renderDirtyCells() {
    ...
    for (const pos of dirtyCells) {
        ...
        const screenX = worldToCanvasX(x);
        const screenY = worldToCanvasY(y);
        
        // Проверка границ
        if (screenX < -CELL_SIZE || screenY < -CELL_SIZE || 
            screenX > offscreenCanvas.width + CELL_SIZE || 
            screenY > offscreenCanvas.height + CELL_SIZE) {
            continue;
        }
...
export function getVisibleSourceRect(camera, canvasWidth, canvasHeight) {
    ...
    // Координаты на оффскрин-канвасе (x,y - индексы клеток, преобразуем в пиксели)
    const sx = worldToCanvasX(startX);
    const sy = worldToCanvasY(startY);
