// map.js — ПОЛНЫЙ С ВИЗУАЛИЗАЦИЕЙ КОТЛОВ
...
export function markDirty() { cacheValid = false; }
...
// ✅ Поиск котлов для визуализации
function findPocketsForRender() {
    const gridData = getGridData();
        ...
        for (const startPos of myCells) {
            ...
            while (queue.length) {
                ...
                for (const [dx, dy] of [[0,1],[0,-1],[1,0],[-1,0]]) {
                    ...
                    if (gridData[neighbor] === countryId) {
...
export function renderMap() {
    ...
    // ========== ВИЗУАЛИЗАЦИЯ КОТЛОВ ==========
    const pockets = findPocketsForRender();
    const now = Date.now();
...
export function setupMapEvents() {
    ...
    canvas.addEventListener('mousemove', e => {
        ...
        if (gd[newHover] && hoverCell !== newHover) {
            hoverCell = newHover;
            cacheValid = false;
            throttledRender();
        } else if (!gd[newHover] && hoverCell) {
            hoverCell = null;
            cacheValid = false;
            throttledRender();
        }
